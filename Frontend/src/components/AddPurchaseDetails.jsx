...
const AddPurchaseDetails = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    // Fetch purchases from local storage
    setPurchases(JSON.parse(localStorage.getItem("purchase")) || []);
  ...
  const handleSave = () => {
    ...
    const selectedProduct = products.find(product => product.name === purchase.name);
    if (selectedProduct) {
      const updatedProducts = products.map((product) => {
        if (product.name === purchase.name) {
          return {
            ...product,
            stock: product.stock + parseInt(purchase.quantityPurchased, 10),
          };
        }
        return product;
      });
      ...
      localStorage.setItem("purchase", JSON.stringify([...savedPurchases, purchase]));
